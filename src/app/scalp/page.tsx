...
export default function Scalp() {
  ...
  const todaysResults = results.filter(r => r.data_saida.startsWith(todayStr));
  const todaysPnl = todaysResults.reduce((sum, r) => sum + (r.resultado_marg || 0), 0);
  ...
  return (
    <div className="animate-in">
      <div className="page-header mb-6">
        <h1>Scalp</h1>
        <p className="page-subtitle">Resultados focados em operações curtas</p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatsCard 
          ...
          label="P&L do Dia (Scalp)" 
          value={`${todaysPnl > 0 ? '+' : ''}${todaysPnl.toFixed(1)}%`} 
          color={todaysPnl < 0 ? 'red' : 'green'} 
        />
      </div>
        ...
        <table>
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={10} className="text-center text-text-muted py-8">Nenhum resultado de scalp encontrado</td>
              </tr>
            ) : (
              results.map(result => (
                <tr key={result.id} className={result.resultado_marg && result.resultado_marg > 0 ? 'bg-green-bg/30' : result.resultado_marg && result.resultado_marg < 0 ? 'bg-red-bg/30' : ''}>
                  ...
                  <td><StatusBadge type="direction" value={result.alerts.direcao} /></td>
